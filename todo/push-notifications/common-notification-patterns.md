...
当收到推送的信息时，通常只有用户点击后才能获取到有用的数据。例如，获取点击通知时需要打开的页面地址.
...
再来看下之前的例子，我们会对代码稍作调整来检测是否页面'/demos/notification-examples/example-page.html'已经被打开。
...
我们会以'异步对象链（promiseChain）'的形式捕捉到返回的promise对象，因此之后就可以将其传入'event.waitUntil()'方法中，并且保持我们的service worker持续工作。
...
还有一点值得提示，如果你不需要在你的页面定义消息监听器，service worker推送的消息将不会做任何事。
...
有一个场景可能超出了这本书的范畴，但是依然值得探讨，那就是你可以在用户点击你希望他去浏览的页面时，缓存这个页面，
以此提升web应用的整体用户体验，
