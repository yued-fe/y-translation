...
```
（自定义属性的工作方式不同 ，在涉及自定义属性的地方，动态作用域意味着它们受继承和级联影响。）
这句话不通，需要再看一下。
属性与选择器是绑定的，如果值发生更改，会影响到其它Dom元素就和默认CSS属性改变一样
...
像一些重要的颜色，排版和间距这样的全局变量，从一个组件到另一个组件的变化不大。当他们改变时，这往往是一个全局品牌重塑或其他一些成熟产品上很少发生的重大变化。这些东西仍然是有意义的变量，它们在很多地方都有使用，变量有助于保持一致性。但是他们的动态是没有意义的。这些变量的值不会以任何动态方式变化。
...
### 何时使用自定义属性
如果可以在组件内使用静态变量，我们应该何时使用自定义属性？将现有的预处理器变量转换为自定义属性通常没什么意义。毕竟，自定义属性的原因完全不同。自定义属性意义的时候，我们有改变相对于DOM中的条件CSS属性-尤其是一个动态的条件，例如:focus，:hovermedia查询或使用JavaScript。

我怀疑我们总是会使用某种形式的静态变量，尽管将来我们可能需要更少的静态变量，因为自定义属性提供了组织逻辑和代码的新方法。在此之前，我认为在大多数情况下，我们将使用预处理器变量和自定义属性的组合。

知道我们可以将静态变量分配给自定义属性很有帮助。无论它们是全局的还是局部的，在很多情况下将静态变量转换为局部动态自定义属性是有意义的。
...
预处理器变量和自定义属性之间的一个主要区别是自定义属性在运行时工作。这意味着在复杂性方面可能被边界接受的事情，对于自定义属性，预处理器可能不是一个好主意。
...
```
这会产生两个CSS文件：一个是自定义属性（styles.css），另一个是旧版浏览器（styles.no-vars.css）。我想要IE10和11 styles.no-vars.css以及其他浏览器来获取常规CSS文件。
...
```
### 结论
如果你一直在努力有效地组织CSS，使用响应式组件很困难，想要实现客户端主题，或者只想顺利地使用自定义属性，那么本指南应该告诉你所需要知道的一切。
