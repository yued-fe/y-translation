...
随着预处理器的引入，我们中的许多人使用mixin和定制函数创建了具有智能抽象的库。在有限的情况下，像这样的例子在今天仍然有用，但是在大多数情况下，我使用预处理器的时间越长，我使用的特性就越少。今天，我几乎只对静态变量使用预处理器。

自定义属性不会(也不应该)不受此类实验的影响，我期待看到许多聪明的示例。但是从长远来看，可读和可维护的代码总是会赢得聪明的抽象(至少在生产中)。
...
```
这就产生了一个模块化的比例。一个模数标度是一系列用比率相互关联的数字。它们通常用于web设计和开发，以设置字体大小或间距。
...
```
这比重新计算所有值(如果您希望更改比例的话)要聪明、简洁，而且要快得多。这也是我在生产代码中不会做的事情。

虽然上面的示例对于原型设计很有用，但是在生产中，我更希望看到如下内容:
``` css
:root {
